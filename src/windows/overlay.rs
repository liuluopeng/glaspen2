...
// Custom window messages for tray commands
pub const WM_TRAY_COMMAND: u32 = WM_USER + 1;

// Command IDs sent via WM_TRAY_COMMAND wParam
...
pub const COLOR_NAMES_EN: [&str; 10] = ["Red", "Orange", "Yellow", "Green", "Cyan", "Blue", "Purple", "Pink", "White", "Black"];

pub const WIDTH_PRESETS: [f64; 5] = [0.3, 0.6, 1.0, 1.5, 2.5];
pub const WIDTH_NAMES_ZH: [&str; 5] = ["极细", "细", "中", "粗", "极粗"];
pub const WIDTH_NAMES_EN: [&str; 5] = ["Fine", "Thin", "Medium", "Thick", "Bold"];

/// Mutable drawing state (colors, cursor, notification).
/// Protected by Mutex since window proc and input hook share it.
...
/// Global overlay window handle, set once the window is created.
/// Used by the tray to send commands via PostMessage.
pub static OVERLAY_HWND: Mutex<isize> = Mutex::new(0);

/// Run the overlay window on the current thread. Blocks until the window is closed.
pub fn run(screen_w: i32, screen_h: i32) {
    ...
    // Create Cairo surface and DIB for UpdateLayeredWindow
    ...
    // Initialize drawing state — load saved settings, fall back to defaults
    ...
    // Install global mouse hook on this thread
    input::install_hook(hwnd);

    // Register hotkeys: Ctrl+Alt+C (clear), Ctrl+Alt+V (toggle), Ctrl+Alt+J (prev page), Ctrl+Alt+K (next page)
    unsafe {
...
unsafe extern "system" fn wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        ...
        WM_USER_NOTIFICATION => {
            ...
            if let Some(data) = data {
                let mut state = data.state.lock().unwrap();
                // Read text from lparam (pointer to String)
...
fn handle_command(state: &mut DrawState, hwnd: HWND, data: &OverlayData, cmd: usize, param: usize) {
    if cmd >= CMD_SELECT_COLOR && cmd < CMD_SELECT_COLOR + 10 {
        ...
        match cmd {
            ...
            x if x == CMD_SAVE_XOJ => {
                crate::glaspen2_save_xoj();
                show_notification(state, hwnd, "Notes saved");
            }
            ...
            x if x == CMD_TOGGLE_RAINBOW => {
                ...
                if state.show_rainbow {
                    render::draw_rainbow_indicator(&data.surface);
                    update_overlay(hwnd);
                } else {
                    clear_screen(state, hwnd, data);
                }
            }
            x if x == CMD_TOGGLE_OUTLINE => {
                ...
                // Rebuild surface to apply/remove outlines
            ...
            x if x == CMD_TOGGLE_LANG => {
                state.lang = 1 - state.lang;
                super::tray::update_menu_texts(state);
            }
            x if x == CMD_QUIT => {
                unsafe {
                    input::uninstall_hook();
...
fn navigate_prev(state: &mut DrawState, hwnd: HWND, data: &OverlayData) {
    ...
    if target == 0 {
        show_notification(state, hwnd, "No previous page");
        return;
    }
    goto_screen(state, hwnd, data, target);
    let current = crate::glaspen2_get_current_screen_id();
    show_notification(state, hwnd, &format!("Page {}/{}", current, current));
}

fn navigate_next(state: &mut DrawState, hwnd: HWND, data: &OverlayData) {
    let target = crate::glaspen2_next_screen_id();
    if target == 0 {
        show_notification(state, hwnd, "No next page");
        return;
    }
    goto_screen(state, hwnd, data, target);
    let current = crate::glaspen2_get_current_screen_id();
...
fn goto_screen(state: &mut DrawState, hwnd: HWND, data: &OverlayData, screen_id: i64) {
    ...
    // Load strokes from DB, smooth, and replay
...
fn rebuild_surface(state: &DrawState, data: &OverlayData) {
    ...
    let strokes = crate::STROKES.lock().unwrap();
    for stroke in strokes.iter() {
        if stroke.points.len() < 1 { continue; }
        // Outline pass
        if state.outline_enabled {
            let (or, og, ob) = render::contrast_color(stroke.r, stroke.g, stroke.b);
            for i in 0..stroke.points.len() {
                let (x, y, pw) = stroke.points[i];
                let extra = f64::max(pw * 0.4, 2.0);
                if i == 0 {
                    render::draw_dot(&data.surface, x, y, pw + extra, or, og, ob);
                } else {
                    let (px, py, _) = stroke.points[i - 1];
                    render::pen_draw(&data.surface, x, y, pw + extra, or, og, ob, px, py, true);
                }
            }
        }
        // Main stroke pass
        for i in 0..stroke.points.len() {
            let (x, y, pw) = stroke.points[i];
            if i == 0 {
                render::draw_dot(&data.surface, x, y, pw, stroke.r, stroke.g, stroke.b);
            } else {
                let (px, py, _) = stroke.points[i - 1];
                render::pen_draw(&data.surface, x, y, pw, stroke.r, stroke.g, stroke.b, px, py, true);
            }
        }
    }
}
...
fn show_notification(state: &mut DrawState, hwnd: HWND, text: &str) {
    ...
    state.notification_time = Some(std::time::Instant::now());
    // Set a timer to clear the notification after 1 second
...
fn save_with_background(_state: &DrawState, data: &OverlayData) {
    // Capture desktop via BitBlt
    let screen_dc = unsafe { GetDC(None) };
    let bw = data.screen_w;
    let bh = data.screen_h;
    ...
    let bg_bmp = unsafe {
        CreateDIBSection(bg_dc, &bmi, DIB_RGB_COLORS, &mut bg_bits, None, 0).unwrap()
    };
    let old_bmp = unsafe { SelectObject(bg_dc, HGDIOBJ(bg_bmp.0 as _)) };
    unsafe {
        BitBlt(bg_dc, 0, 0, bw, bh, screen_dc, 0, 0, SRCCOPY).unwrap();
    }

    // Get drawing surface data
    let draw_w = data.surface.width();
    let draw_h = data.surface.height();
...
/// Update the layered window with current Cairo surface contents.
pub fn update_overlay(hwnd: HWND) {
    let data = match get_overlay_data(hwnd) {
        Some(d) => d,
        None => return,
    };

    // Copy stroke surface to DIB (no lock needed — surface is separate from state)
    ...
    // Read cursor/notification state under lock
    ...
    // Draw overlays on the DIB via Cairo on a temporary surface, then blend into DIB
    if cursor_visible || notification.is_some() {
        ...
        {
            if cursor_visible && cursor_x >= 0.0 {
                render::draw_crosshair(&overlay_surface, cursor_x, cursor_y);
            }
            if let Some(ref text) = notification {
                render::draw_notification(&overlay_surface, text);
            }
        }
        // Blend overlay pixels into the DIB
        ...
        for y in 0..data.screen_h as usize {
            for x in 0..data.screen_w as usize {
                ...
                if o_off + 3 < overlay_data.len() && d_off + 3 < dib.len() {
                    ...
                    if alpha > 0.01 {
                        // Premultiplied alpha compositing: dst = src + dst * (1 - alpha)
    ...
    unsafe {
        UpdateLayeredWindow(
            hwnd,
            None,
            Some(&point as *const POINT),
            Some(&size as *const SIZE),
            data.hdc_mem,
            Some(&point_src as *const POINT),
            COLORREF(0),
