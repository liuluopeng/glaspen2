...
struct StrokeModelerState {
    ...
    start_time: f64,
    buffer: Vec<(f64, f64, f64, f64)>, // (x, y, pressure→width, relative_time)
}
...
/// Returns smoothed (x, y, width, estimated_time) points.
/// Each smoothed point estimates time from the nearest raw input point.
pub fn smooth_points(points: &[(f64, f64, f64)]) -> Vec<(f64, f64, f64, f64)> {
    if points.len() < 2 {
        // Convert 3-tuples to 4-tuples with zero time
    ...
    let mut result = vec![(x0, y0, w0, 0.0f64)];
